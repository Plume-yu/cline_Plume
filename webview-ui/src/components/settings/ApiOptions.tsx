			...
			<DropdownContainer className="dropdown-container">
						...
						<TooltipTrigger>
							<div className="flex items-center gap-2 mb-1">
								<label htmlFor="api-provider">
									<span style={{ fontWeight: 500 }}>{t("apiOptions.apiProvider")}</span>
								</label>
								<i className="codicon codicon-lock text-description text-sm" />
							</div>
						</TooltipTrigger>
						<TooltipContent>{t("apiOptions.apiProviderLocked")}</TooltipContent>
					</Tooltip>
				) : (
					<label htmlFor="api-provider">
						<span style={{ fontWeight: 500 }}>{t("apiOptions.apiProvider")}</span>
				...
				<ProviderDropdownWrapper ref={dropdownRef}>
					<VSCodeTextField
						...
						placeholder={t("apiOptions.selectProvider")}
						role="combobox"
						style={{
							width: "100%",
